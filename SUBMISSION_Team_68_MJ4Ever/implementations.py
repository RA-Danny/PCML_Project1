...
def least_squares_SGD(y, tx,initial_w, max_iters, gamma):
    ...
    return w, compute_loss(y_batch,tx_batch,w)
...
def ridge_regression(y, tx, lambda_):
    ...
    return weight, compute_loss(y, tx, weight)
...
def build_poly2(x, degree):
    """polynomial basis functions for input data x, for j=0 up to j=degree.
       This version profites more features for a matrix data"""
    x_poly = np.ones(np.shape(x))
...
def remove_column(tx,column_to_remove):
    """Remove some specified columns for all matrices given in an array"""
    tX_clean = []
    for i in range(len(tx)):
        x = tx[i]

        column_to_keep = [i for i in range(np.shape(tx[i])[1])]
        column_to_keep = np.delete(column_to_keep,column_to_remove[i])
